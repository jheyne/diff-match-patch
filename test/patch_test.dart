...
main() {
  group("Patch", () {
    test("toString", () {
      ...
      p.length1 = 18;
      p.length2 = 17;
      p.diffs = [new Diff(DIFF_EQUAL, 'jump'), new Diff(DIFF_DELETE, 's'),
                 new Diff(DIFF_INSERT, 'ed'), new Diff(DIFF_EQUAL, ' over '),
                 new Diff(DIFF_DELETE, 'the'), new Diff(DIFF_INSERT, 'a'),
                 new Diff(DIFF_EQUAL, '\nlaz')];
    ...
    group("Make", () {
      ...
      test("Character encoding", () {
        var patches = patchMake('`1234567890-=[]\\;\',./',
            b: '~!@#\$%^&*()_+{}|:"<>?');
        expect(patchToText(patches), equals('@@ -1,21 +1,21 @@\n-%601234567890-=%5B%5D%5C;\',./\n+~!@#\$%25%5E&*()_+%7B%7D%7C:%22%3C%3E?\n'));
      });

      test("Character decoding", () {
        var diffs = [new Diff(DIFF_DELETE, '`1234567890-=[]\\;\',./'), new Diff(DIFF_INSERT, '~!@#\$%^&*()_+{}|:"<>?')];
        expect(patchFromText('@@ -1,21 +1,21 @@\n-%601234567890-=%5B%5D%5C;\',./\n+~!@#\$%25%5E&*()_+%7B%7D%7C:%22%3C%3E?\n')[0].diffs, equals(diffs));
      });
    ...
    group("Split Max", () {
      ...
      test("#1", () {
        ...
        expect(patchToText(patches), equals('@@ -1,32 +1,46 @@\n+X\n ab\n+X\n cd\n+X\n ef\n+X\n gh\n+X\n ij\n+X\n kl\n+X\n mn\n+X\n op\n+X\n qr\n+X\n st\n+X\n uv\n+X\n wx\n+X\n yz\n+X\n 012345\n@@ -25,13 +39,18 @@\n zX01\n+X\n 23\n+X\n 45\n+X\n 67\n+X\n 89\n+X\n 0\n'));
      });
      ...
      test("#3", () {
        var patches = patchMake('1234567890123456789012345678901234567890123456789012345678901234567890',
            b: 'abc');
        patchSplitMax(patches);
        expect(patchToText(patches), equals('@@ -1,32 +1,4 @@\n-1234567890123456789012345678\n 9012\n@@ -29,32 +1,4 @@\n-9012345678901234567890123456\n 7890\n@@ -57,14 +1,3 @@\n-78901234567890\n+abc\n'));
      });
      ...
      test("#4", () {
        ...
        patchSplitMax(patches);
        expect(patchToText(patches), equals('@@ -2,32 +2,32 @@\n bcdefghij , h : \n-0\n+1\n  , t : 1 abcdef\n@@ -29,32 +29,32 @@\n bcdefghij , h : \n-0\n+1\n  , t : 1 abcdef\n'));
      });
    });
    ...
    group("Apply", () {
      ...
      test("Big delete, small change", () {
        var patches = patchMake('x1234567890123456789012345678901234567890123456789012345678901234567890y',
            b: 'xabcy');
        var results = patchApply(patches, 'x123456789012345678901234567890-----++++++++++-----123456789012345678901234567890y');
        var boolArray = results[1];
        var resultStr = '${results[0]}\t${boolArray[0]}\t${boolArray[1]}';
      ...
      test("Big delete, big change #1", () {
        var patches = patchMake('x1234567890123456789012345678901234567890123456789012345678901234567890y',
            b: 'xabcy');
        var results = patchApply(patches, 'x12345678901234567890---------------++++++++++---------------12345678901234567890y');
        var boolArray = results[1];
        var resultStr = '${results[0]}\t${boolArray[0]}\t${boolArray[1]}';
        expect(resultStr, 'xabc12345678901234567890---------------++++++++++---------------12345678901234567890y\tfalse\ttrue');
      });
